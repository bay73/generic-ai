...
data class CohereHttpChatResponseFormat(
    ...
    companion object {
        fun from(genericResponseFormat: ResponseFormat?): CohereHttpChatResponseFormat? =
            when (genericResponseFormat?.type) {
                null -> null
                ResponseFormat.Type.TEXT -> CohereHttpChatResponseFormat("text")
                ResponseFormat.Type.JSON_OBJECT -> CohereHttpChatResponseFormat("json_object")
